...
def build_parts_list_html(parts: list[dict]) -> str:
    ...
    for part in parts:
        ...
        if first_image:
            has_any_images = True
        normalized_parts.append((part, first_image))

    if has_any_images:
        lines = ['<div class="parts-card-grid">']
        for part, first_image in normalized_parts:
            name = part.get("name", "Unnamed Part")
            role = part.get("role", "").strip()
            url = part.get("url", "").strip()
            ...
            name_html = html_link(name, url) if url else safe_name
            view_part_link = (
                f'<a class="part-card-link" href="{escape(url, quote=True)}">View part</a>'
                if url
                else ""
            )
            ...
            if first_image:
                image_html = (
                    f'<img class="part-card-image" src="{escape(first_image, quote=True)}" '
                    f'alt="{safe_name}">'
                )

            lines.append('  <article class="part-card">')
            lines.append(f"    {image_html}" if image_html else "    <div class=\"part-card-image\"></div>")
            lines.append('    <div class="part-card-body">')
            lines.append(f'      <h3 class="part-card-title">{name_html}</h3>')
            if safe_role:
                lines.append(f'      <p class="part-card-role">{safe_role}</p>')
            if view_part_link:
                lines.append(f"      {view_part_link}")
            lines.append("    </div>")
    ...
    for part, _ in normalized_parts:
        name = part.get("name", "Unnamed Part")
        role = part.get("role", "").strip()
        url = part.get("url", "").strip()
