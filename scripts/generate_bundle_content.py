...
def build_parts_list_html(parts: list[dict]) -> str:
    ...
    for part in parts:
        ...
        if role:
            lines.append(
                f'  <li><strong>{display_name}</strong> — {escape(role.capitalize())}</li>'
            )
        else:
...
def build_blog_context(bundle: dict) -> dict:
    ...
    return {
        ...
        "why_intro": (
            "A good upgrade bundle should do more than look good on paper. "
            "It should help answer where to start and what actually makes a difference."
        ),
        "benefit_1": "Take hits better",
        "benefit_2": "Stay tighter",
        "benefit_3": "Keep power where it belongs",
        "benefit_4": "Hold up run after run",
